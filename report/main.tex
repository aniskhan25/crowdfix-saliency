...
\begin{table*}[!t]
  ...
  \caption{%
    ...
    ``--'': metric not reported in the original publication.
    \textbf{Bold (mean)}: best per column; \underline{underline}: second best.
    ...
    the two rows differ by $\leq\!0.001$ on every metric, confirming the oracle
    dependency does not affect deployment performance.
  }
  \resizebox{\textwidth}{!}{%
...
\begin{table}[!t]
  ...
  \caption{%
    ...
    $\Delta$NSS is measured relative to the VideoSwin baseline.
    The ``+Density FiLM'' row reports seed~42 values; Table~\ref{tab:sota}
    reports mean\,$\pm$\,std over 4 seeds.  \textbf{Bold}: best.
  }
  ...
  \begin{tabular}{lcccc}
    ...
    +~Density FiLM (\textbf{ours})
      & \textbf{0.820} & \textbf{1.437} & \textbf{0.517} & $+$0.124 \\
    \midrule
    +~One-hot density code (no FiLM)
      & 0.819 & 1.424 & 0.514 & $+$0.111 \\
    +~Optical flow (RAFT)
...
CC\,0.517 (seed~42).  Two findings follow.  First, the conditioning
\emph{signal} is the primary driver: the one-hot model captures
$\Delta$NSS\,=\,$+0.111$ of FiLM's $+0.124$ total gain over the unconditioned
backbone (89\% of the improvement).  Second, the FiLM \emph{mechanism}
provides a real but modest additional benefit: $+0.013$ NSS
(${\approx}1.9\times$ the per-seed standard deviation of $0.007$),
attributable to the 64-dim embedding geometry and the multiplicative
$\gamma$ pathway enabling class-specific \emph{scaling} rather than mere additive
shifting.  This $+0.013$ NSS advantage corresponds to ${\approx}1.9$
seed standard deviations and should be read as indicative rather than
definitive at conventional significance thresholds; the near-orthogonal
$\gamma$ geometry (Section~\ref{sec:film_analysis}) provides
geometrically independent support that FiLM learns genuinely distinct
class-specific representations beyond what an additive bias achieves.
Together these results confirm that crowd density is a genuine
conditioning variable, and that the richer FiLM parameterisation
extracts slightly more signal than a direct one-hot bias.
...
over the unconditioned backbone, while three progressively more complex
extensions all fail to improve, with the most complex variant collapsing NSS
precisely to baseline.  The result is a new state of the art on CrowdFix
...
been tested.
Third, \textbf{residual centre bias}: the VideoSwin encoder inherits a
centre-biased prior from Kinetics-400 pretraining, and the CrowdFix
fixation maps exhibit a corresponding centre tendency.  Challenging frames
in the qualitative analysis (Figure~\ref{fig:qualitative}) confirm this
is the primary remaining failure mode: in all three negative-NSS examples
the model predicts a central saliency peak while observers fixated an
off-centre target.  The NSS values reported in this paper therefore
conflate density-conditioning benefit with shared centre-prior
exploitation.  A debiased NSS analysis---subtracting the mean
training-set fixation density from all predictions before metric
computation---is required to isolate the two contributions and is
planned as the next evaluation step.
