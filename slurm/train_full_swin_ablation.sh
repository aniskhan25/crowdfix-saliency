#!/bin/bash
# Ablation baseline: vanilla VideoSwin (no density conditioning).
# Matches run 4 hyperparameters exactly — same epochs, LR, dropout structure —
# to isolate the contribution of density-conditioned FiLM.
#
# Usage: cd <repo-root> && sbatch slurm/train_full_swin_ablation.sh

#SBATCH --job-name=crowdfix-swin-ablation
...
#SBATCH --output=logs/train_full_swin_ablation_%j.log
...
time srun singularity exec -B "$SQSH_PATH":/user-software:image-src=/ "$CONTAINER" \
  ...
  train.py \
    ...
    --data-dir             "$DATA_DIR" \
    --splits               splits.json \
    --checkpoint-dir       checkpoints_swin \
    --epochs               150 \
    --batch-size           2 \
    --lr                   1e-4 \
    --clip-len             8 \
