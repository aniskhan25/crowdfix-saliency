#!/bin/bash
# Smoke-test run for density_swin model on dev-g (1 node, 8 GCDs, 30 min max).
#
# Usage: source env.sh && sbatch slurm/train_dev_density.sh

#SBATCH --job-name=crowdfix-density-dev
...
#SBATCH --output=logs/train_dev_density_%j.log
