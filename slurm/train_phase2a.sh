#!/bin/bash
# Phase 2a: DensitySwinSoft — continuous (self-supervised at inference) bottleneck FiLM.
# Same hyperparams as Run 4 (dropout=0.5, freeze 20, patience 20, 150 epochs).
#
# Usage: cd <repo-root> && sbatch slurm/train_phase2a.sh

#SBATCH --job-name=crowdfix-soft
...
#SBATCH --output=logs/train_phase2a_%j.log
...
time srun singularity exec -B "$SQSH_PATH":/user-software:image-src=/ "$CONTAINER" \
  ...
  train.py \
    ...
    --data-dir             "$DATA_DIR" \
    --splits               splits.json \
    --checkpoint-dir       checkpoints/soft \
    --epochs               150 \
    --batch-size           2 \
    --lr                   1e-4 \
    --clip-len             8 \
