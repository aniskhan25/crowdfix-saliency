#!/bin/bash
# Run 4: v1 architecture, dropout=0.5, early stopping patience=20, 150 epochs.
# ~40 s/epoch × 150 = ~100 min; 4h gives comfortable headroom.
#
# Usage: cd <repo-root> && sbatch slurm/train_full_density_r4.sh

#SBATCH --job-name=crowdfix-density-r4
...
#SBATCH --output=logs/train_full_density_r4_%j.log
...
# Base dir on /tmp — train.py appends LOCAL_RANK so each rank has its own subdir.
