#!/bin/bash
# Smoke-test run on dev-g (1 node, 8 GCDs, 30 min max).
# Runs 2 training steps to verify DDP init, data loading, and forward/backward.
#
# Usage: source env.sh && sbatch slurm/train_dev.sh

#SBATCH --job-name=crowdfix-dev
...
#SBATCH --output=logs/train_dev_%j.log
...
# MIOpen cache must be on /tmp, not Lustre (file locking incompatibility)
